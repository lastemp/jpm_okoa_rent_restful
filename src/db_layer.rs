...
pub fn create_ussd_session_details(
    data: &web::Data<Pool>,
    session_id: String,
    ...
    match data.get_conn().and_then(|mut conn| {
        insert_ussd_session_details(&mut conn, session_id, caller_number, caller_action)
    }) {
        Ok(x) => {
            successful = true;
        }
        Err(e) => println!(
            "Failed to open DB connection. create_ussd_session_details {:?}",
            e
        ),
    }
...
pub fn create_okoa_rent_data(
    ...
    match data.get_conn().and_then(|mut conn| {
        insert_okoa_rent_data(&mut conn, tenant_code, house_code, amount, mobile_no)
    }) {
        Ok(x) => {
            if x > 0 {
                successful = true;
...
pub fn create_okoa_mortgage_data(
    data: &web::Data<Pool>,
    mortgagor_code: String,
    house_code: String,
    amount: i32,
    mobile_no: String,
) -> bool {
    ...
    match data.get_conn().and_then(|mut conn| {
        insert_okoa_mortgage_data(&mut conn, mortgagor_code, house_code, amount, mobile_no)
    }) {
        Ok(x) => {
            if x > 0 {
                successful = true;
...
pub fn create_check_balance_data(
    data: &web::Data<Pool>,
    rent_mortgage_code: String,
    mobile_no: String,
) -> bool {
    let mut successful: bool = false;
...
pub fn create_get_statement_data(
    data: &web::Data<Pool>,
    full_statement: bool,
    mini_statement: bool,
    mobile_no: String,
    ...
    match data.get_conn().and_then(|mut conn| {
        insert_get_statement_data(&mut conn, full_statement, mini_statement, mobile_no)
    }) {
        Ok(x) => {
            if x > 0 {
                successful = true;
...
pub fn create_self_registration_data(
    data: &web::Data<Pool>,
    ...
    match data.get_conn().and_then(|mut conn| {
        insert_self_registration_data(&mut conn, national_id_no, full_names, house_code, mobile_no)
    }) {
        Ok(x) => {
            if x > 0 {
                successful = true;
...
pub fn create_outgoing_sms_message_data(
    data: &web::Data<Pool>,
    sms_message: String,
    _to: String,
    _from: String,
    ...
    match data.get_conn().and_then(|mut conn| {
        insert_outgoing_sms_message_data(
            &mut conn,
            sms_message,
            _to,
            _from,
...
pub fn get_ussd_session_details(
    data: &web::Data<Pool>,
    session_id: &String,
    phone_number: &String,
) -> String {
    ...
    match data.get_conn().and_then(|mut conn| {
        select_ussd_session_details(&mut conn, session_id.to_string(), phone_number.to_string())
    }) {
        Ok(x) => {
            caller_action = x;
        }
